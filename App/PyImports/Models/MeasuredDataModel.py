...
class MeasuredDataModel(BaseModel):
    ...
    def _setModelsFromProjectDict(self):
        ...
        """
        logging.info("set measured data model: start")
        ...
        logging.info("set measured data model: end")

    def _updateQmlChartViewSeries(self):
        """
        Updates QML LineSeries of ChartView.
        """
        logging.info("update measured data series: start")

        # Indices of the self._model columns to be plotted on chart
        x_column = 0                                        # first column in experiment_dict['measured_pattern']
        y_obs_upper_column = self._model.columnCount() - 2  # before last column in experiment_dict['measured_pattern']
        y_obs_lower_column = self._model.columnCount() - 1  # last column in experiment_dict['measured_pattern']
        ...
        for row_index in range(self._model.rowCount()):
            ...
            index = self._model.index(row_index, y_obs_upper_column)
            y_obs_lower_list.append(self._model.data(index))

            index = self._model.index(row_index, y_obs_lower_column)
        ...
        logging.info("update measured data series: end")
